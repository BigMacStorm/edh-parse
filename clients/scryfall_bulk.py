...
BULK_CACHE_DIR = Path("cache/scryfall_bulk_cache")
...
def ensure_loaded(session, force_refresh=False):
    ...
    is_current = (
        ...
        and local_updated_at == updated_at
    )
    if is_current:
        if _conn is None:
    ...
    need_download = force_refresh or (not gzip_exists) or (local_updated_at != updated_at)
    if need_download:
    ...
    if not db_exists or need_download:
